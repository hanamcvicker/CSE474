...
#define GPIO_IS_N (*((volatile uint32_t *)0x40064404)) //Interrupt Sense register
...
#define GPIO_MIS_N (*((volatile uint32_t *)0x40064418)) // use this for edge detect interrupts, 
...
// Initializes the two switches on the board used to change the frequency preset to 
// 12 MHz or 120 MHz depending on which switch is pressed.
void switches_Init();
...
// Handler to calculate and print the temperature from the board.
